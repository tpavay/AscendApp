...
- `target_reached` is the only stop reason that earns competitive credit, and it is the gate the server-side replay indexer publishes on. Any future routine leaderboard publisher must keep gating on it rather than inventing a parallel eligibility rule.
