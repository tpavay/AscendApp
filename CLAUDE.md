...
- Discovery only reaches the `users/{uid}` subtree. User-keyed PII stored outside it needs an explicit step in the sweep, so `leaderboard_stats`, `userRateLimits`, and `live_replay_leaderboards/{contextKey}/finishers/{uid}` each get one. Add a step whenever a new collection denormalizes a user's public display fields; the `firstAscent*` fields on the replay context root are the deliberate exception, since a First Ascent holder stays associated with the climb forever.
