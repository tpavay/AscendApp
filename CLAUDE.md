...
- **A surface Ascend draws itself is invisible to Sentry's masking, so it reaches a crash screenshot and a replay frame legibly.** The SDK covers text, images, SF Symbols, web views and `AVPlayerView`; it does not cover Swift Charts marks *or their axis labels*, `Canvas`/`Shape` drawing that encodes a measurement, or `AVPlayerLayer`-backed video. A masked screenshot of Workout Detail still showed the whole heart-rate trace with real BPM values on the axis until `View.sentryMasked()` was applied. Apply it inside the view's own definition, never at call sites, and prove it in `AscendAppTests/SentryReplayMaskingEvidenceTests.swift`. Fires while adding a chart or a media player, which does not look like privacy work. -> `docs/sentry-setup.md`
