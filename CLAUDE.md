...
- **Heart-rate enrichment keeps looking on its bounded schedule even when celebratory surfaces stay silent.**
  ...
  `AppleHealthEnrichmentService.Phase` is the one answer for any surface that narrates checking, waiting, stopped looking, checks paused, connection, revoked access, unavailable, or not applicable, but the completion summary and Workout Detail deliberately show the chart or nothing.
  Settings -> Integrations is the only connection and revoked-access surface.
