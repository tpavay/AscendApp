...
- **A rank counts people, and every surface says which population it counted.** One row per unique climber at their best, ranked on that climb's own time; your previous best is the `BEST` marker inside your own row and is never a row, never in the rank, never in the field size. A finished summary states where you stood *then* and never re-computes, so it reads the same reopened and on the share card. Climb Detail is the opposite surface on purpose - all of your attempts and everybody else's, titled `ALL TIMES`. Which **rows** a board draws is not which population a **rank** counts: Climb Detail draws every completed attempt as its own row, ranks those rows as attempts, and counts completions (`41 COMPLETIONS`), while the finish summary's recomputed standing for that same climb counts unique climbers on both halves (41 finishes from 16 climbers with 5 climbers ahead of you is `6TH OF 16`). Counting differently on different screens is correct; counting one population and naming another is the defect. Fires while writing a rank sentence, a field-size line, a hero, a share card, or a Cloud Function that freezes a standing - none of which look like each other. -> `ascend-leaderboards` (The rank model)
...
- **Clearing the analytics identity is only ever a real signed-in -> signed-out transition, and a simulator is never a production customer.** Firebase's auth listener reports "no user" on every signed-out cold launch too, and answering it with `Mixpanel.reset()` mints a new random device id that the ID merge can never rejoin - it severed 19 of 37 climbers from their own `app_first_opened` and was the entire 37 -> 9 cliff on the onboarding board. `TelemetryManager` owns that state machine through `TelemetryIdentityStore`, which survives relaunch so a revoked session still counts as a sign-out; `clearUserId()`'s return value is the app's only answer to "did somebody sign out". Separately, a Release binary compiled for the simulator SDK reports `production`/`release` exactly like a customer's, which put 34 simulator identities into the production Mixpanel project in one afternoon, so `TelemetryRuntimeEnvironment` refuses production from a simulator at both the collection gate and the destination guard - staging is where a session is reproduced. Fires while editing auth code or reasoning about build configurations, neither of which looks like analytics work. -> `ascend-analytics`
...
- **A filled image reports the size it scaled to, not the size it was offered, and `.clipped()` bounds only the drawing.** A landscape hero photo filling a phone's height is several phones wide, so a `.resizable().scaledToFill()` `Image` placed as a ZStack sibling widens the whole stack and every view drawn over it lays out at the photo's width, centred, spilling off both screen edges - which is how the Just Me tab lost its chrome and stat row the day #582 put the climb photo behind it. Draw a fill as an `.overlay` on a view that takes exactly the proposal (`Color.clear`), the way `ClimbArtworkView` does, and never trust `.clipped()` or `.frame(maxWidth: .infinity)` to bound it. `AscendAppTests/LiveClimbJustMePhotoBackgroundWidthTests.swift` pins the component's size contract and the screen. Fires while adding a photo or artwork background to any screen, which looks like styling rather than layout.
...
- **A refusal that must not be escapable is a route, never a sheet, and the hard update lockout is resolved first - above authentication.** A presented paywall sits in **its own `UIWindow` above the app's**, so nothing Ascend raises - sheet, alert or dialog - can appear over it, and a second sheet at the same modifier level defers the first: the lockout sheet was invisible to exactly the climber most likely to be on a stale build, the unentitled one who cold-starts into the paywall (#429), and the same window rule is why the paywall's `DELETE ACCOUNT` control has to be dismissed by Ascend rather than opening the deletion dialog over the paywall (#558). A surface that must be reachable *while* a paywall is up is drawn by the paywall and answered by the app, never presented under it - the verified mechanism and the two sanctioned shapes are in `docs/superwall-paywall-setup.md`. When the app must raise one of those sheets and another may already be live, the live one *yields* and hands the request on from its own `onDismiss` - the soft nudge yields to account deletion - because detecting the clash and reporting a refusal only moves the dead end: the gate that asked has already dismissed its paywall and has nowhere to render the refusal. Order is lockout -> auth -> entitlement -> onboarding -> main app, and `AppRootRouteResolver` stays synchronous and pure - the Remote Config and RevenueCat reads happen outside it, so cold start and foreground run the identical decision. The floor is the last one the device *received*, seeded from the persisted activation before any fetch: hitting the lockout online means hitting it offline, and only a device the backend never answered fails open. The soft nudge stays a dismissible sheet. Fires while adding any modal, redesigning the gate, or writing a "block the app" state. -> `AscendApp/App/AppRootRoute.swift`, `AscendApp/Shared/Services/RemoteConfig/AppVersionGateState.swift`
...
- **Creating a Functions secret version is a production change that the next unrelated deploy ships.** A Functions deploy binds every function to each secret's *latest* version, so `REVENUECAT_SERVER_CONFIG` version 3 - built on 2026-09-08 from a stale local copy that had lost `rc_promo_app_access_lifetime`, and left undeployed - went live with the 1.1 deploy on 2026-09-25, and reconciliation deleted comped climbers' `app_access` grants. Never build a secret version from a local copy, the Keychain included: build it from the version the deployed functions are bound to, change only the field you mean to, and pin it in `functions/secret-versions.json` in the same reviewed PR. Both deploys refuse a latest version the commit does not pin, refuse a RevenueCat allowlist that drops a product the app sells, comps with, or a live grant holds, or that a bound version allowlists without a committed acknowledgement for that product and pinned version, and stop before rules if any pre-deploy grant would not survive; disabling an unwanted version is not an undo, because `latest` still resolves to it. Fires while rotating a credential or editing an allowlist in a terminal, which does not look like deploy work. -> `docs/functions-secret-versions.md`
...
- **A test that reads a screen reads it through `RenderedScreen`, never through a private `UIWindow`, `drawHierarchy`, `ImageRenderer` or Vision helper.** Eighty suites each hosted a full screen privately, photographed it at 3x (12 MB a bitmap), OCR'd the bitmap to prove copy was on screen, and kept the image for the length of the test; the host never gave that memory back, fourteen suites peaked over 1 GB, and `iOS Verify (Staging)` could not hold the suite in one process. Copy presence and absence are accessibility-tree facts (`screen.copy()`, which fails on a missing, hidden or off-screen label the way OCR did); OCR is for legibility only - an accessibility text size, a mask proof, visible copy a custom `accessibilityLabel` replaces - and stays behind `recognizedText(scale:)`; pixel reads are 1x unless the read is about glyphs; the 3x photograph is written only when `ASCEND_EVIDENCE_DIR` is set, and CI never sets it. A suite that hosts carries `.hostsAWindow`. Fires while writing an evidence test, which looks like feature work. -> `AscendAppTests/RenderedScreen.swift`
- **Shipping means bumping `MARKETING_VERSION` in the same pull request, and no automation may press Submit.** Apple closes a version train the moment that version goes live, so a merge to `main` still declaring a released version deploys Firestore, Functions, rules and Hosting successfully and *then* fails the app upload - a silent half-release, which is what run 32886787708 did on 2026-08-25. Everything up to a prepared App Store version record with its build attached is automated (`prepare-app-store-version.yml`); submitting for review, releasing, and steering a phased rollout stay the captain's, enforced by `scripts/test/app-store-submission-guard.test.mjs`. Fires while merging to `main` or writing release automation, neither of which looks like App Store Connect work. -> `docs/release-procedure.md`, `ascend-deploy`
...
- `functions/secret-versions.json` - the Functions secret version each project must bind; a deploy refuses any other (`docs/functions-secret-versions.md`)
