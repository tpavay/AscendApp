...
- **Anything Sign in with Apple already supplied may never be asked for again, and Apple supplies it exactly once.** 1.0 was rejected under Guideline 4 on 2026-08-20 because the request asked for `[.email]` only, so `fullName` was always nil and the name step ran for every Apple climber. `ASAuthorizationAppleIDCredential.fullName` and `.email` are populated on the FIRST authorization for an Apple ID and app pair and never again, so the credential is captured before anything else can fail (`AppleSignInIdentityStore`, which survives sign-out on purpose) and adopted onto a profile that has none - never over one that already has a name, and never on a profile read that failed. A relay `@privaterelay.appleid.com` address is a real address. A climber who shared only half a name is asked for the half they withheld, seeded with the half they gave. Fires while adding any onboarding field, editing the auth request's scopes, or reordering the post-auth stages - none of which look like App Review work. -> `AscendApp/Features/Authentication/AppleSignInSuppliedIdentity.swift`, `app-store-review`
