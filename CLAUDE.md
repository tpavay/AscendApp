...
- **A name any source already supplied may never be asked for again, and the app may never block on one.** 1.0 was rejected under Guideline 4 on 2026-08-20 because the Apple request asked for `[.email]` only, so `fullName` was always nil and the name step ran for every Apple climber. The rule is provider-agnostic on purpose - Google supplies a display name too - and resolves in one order: the profile Ascend already holds, then whatever this sign-in supplied, then `SignInNamePlaceholder` ("CHANGE ME"). `ASAuthorizationAppleIDCredential.fullName` and `.email` are populated on the FIRST authorization for an Apple ID and app pair and never again, so the credential is captured before anything else can fail (`SignInIdentityStore`, which survives sign-out on purpose) and adopted onto a profile that has none - never over one that already has a name, and never on a profile read that failed. A relay `@privaterelay.appleid.com` address is a real address. A climber who shared only half a name is asked for the half they withheld, seeded with the half they gave. App Review's device has already spent its one first authorization, so a name screen with no way past is the same rejection again however good the prefill is: the step carries a real Skip and never a dimmed CONTINUE alone. Fires while adding any onboarding field, editing the auth request's scopes, or reordering the post-auth stages - none of which look like App Review work. -> `AscendApp/Features/Authentication/SignInSuppliedIdentity.swift`, `app-store-review`
