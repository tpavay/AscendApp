...
- That split exists because the opposite cost a live release. The paywall sends `String(describing: paywallInfo.closeReason)`, which prints a Swift enum as `manualClose`, and for two months every production paywall dismissal was rejected outright with `reason must be a lifecycle key.` (Sentry ASCEND-IOS-1). An optional field the server is happy to do without may not decide whether the surrounding event survives, and a value it had to repair may not vanish silently. Repairing the value is the server's safety net, not the contract: #527 tracks giving the client an owned key so nothing needs repairing.
