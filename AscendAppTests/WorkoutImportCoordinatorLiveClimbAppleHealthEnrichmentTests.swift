...
struct WorkoutImportCoordinatorLiveClimbAppleHealthEnrichmentTests {
    ...
    func refreshEnrichesSingleLiveClimbWithSingleAppleHealthWorkout() async throws {
        ...
        let coordinator = WorkoutImportCoordinator(
            ...
            workoutReader: LiveClimbHealthKitWorkoutReader(workouts: [appleWorkout], addedSamples: [appleSample]),
        ...
        #expect(liveWorkout.sourceLink(for: .appleHealth)?.externalRecordID == appleSample.externalRecordID)
        ...
        #expect(metricsReader.requestedRanges[appleSample.externalRecordID]?.upperBound == liveStart.addingTimeInterval(480))
    }
    ...
    func refreshEnrichesTwoLiveClimbsWithTwoDistinctAppleHealthWorkouts() async throws {
        ...
        modelContext.insert(makeLiveClimbParticipation(for: secondLiveWorkout, climbId: "second-climb"))
        try modelContext.save()
    ...
    func refreshDoesNotLinkOneAppleHealthWorkoutToMultipleLiveClimbs() async throws {
        ...
        let coordinator = WorkoutImportCoordinator(
            authorizationController: LiveClimbHealthKitAuthorizationController(),
            workoutReader: LiveClimbHealthKitWorkoutReader(workouts: [longAppleWorkout], addedSamples: [sample]),
            metricsReader: LiveClimbHealthKitMetricsReader()
        )
        coordinator.configure(modelContext: modelContext)
        ...
        #expect(coordinator.pendingCandidates.first?.appleHealthSample?.externalRecordID == sample.externalRecordID)
    }
    ...
    private func makeLiveClimbParticipation(for workout: Workout, climbId: String) -> WorkoutParticipation {
...
private final class LiveClimbHealthKitMetricsReader: HealthKitMetricsReading {
    ...
    init(metrics: WorkoutMetrics = WorkoutMetrics(avgHeartRate: 135, maxHeartRate: 165, caloriesBurned: 75)) {
