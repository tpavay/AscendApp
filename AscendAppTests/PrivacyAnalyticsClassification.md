...
| Product Interaction | `screen_id`, `step_id`, `from_step`, `flow_id`, `flow_version`, `segment_id`, `step_index`, `step_count`, `resume`, `completion_reason`, `action_id`, `input_type`, `selection_type`, `answer_index`, `has_answer`, `viewed`, `completed`, `question_id`, `provider`, `source`, `entry_point`, `surface`, `share_surface`, `card_type`, `home_state`, `action_state`, `can_start`, `blocked_reason`, `selection_method`, `display_name_provided`, `total_climbs_bucket`, `filter_group`, `filter_type`, `metric`, `time_frame`, `active_filter_count`, `has_active_filters` | Navigation, funnel position, and interaction shape across every feature |
| Other Diagnostic Data | `app_environment`, `build_config`, `app_version`, `build_number` | The `TelemetryEnvelope` the facade attaches to every event and every screen view, so each one names the build that emitted it. Build metadata only - it carries no user information |
...
- `outcome` is **Fitness** on `workout_import_finished` and `live_climb_attempt_saved`, and **Purchase History** on `revenuecat_purchase_completed` and `revenuecat_restore_completed`.
