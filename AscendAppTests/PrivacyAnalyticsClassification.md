...
| Other Diagnostic Data | `holds_sandbox_entitlement`, `storekit_receipt_name` | The StoreKit environment pair `StoreKitEnvironmentDiagnostics` merges onto every `paywall_*` and `revenuecat_*` event. A boolean about the running install and the receipt file name iOS handed it (`sandboxReceipt`, `receipt`, or `none`) - device environment facts that name no product, price, account, or purchase, which is why they are Diagnostic rather than Purchase History |
