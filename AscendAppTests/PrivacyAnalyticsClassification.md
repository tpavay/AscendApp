...
| Fitness | `headphone_family`, `headphone_output_connected`, `motion_capable_headphone_connected`, `headphone_motion_data_flowed`, `app_steps_bucket`, `machine_steps_bucket`, `discrepancy_abs_bucket`, `discrepancy_percent_bucket` | `WorkoutStepAccuracyAnalyticsEvent` (`step_accuracy_telemetry_recorded`, `step_accuracy_calibration_submitted`, `step_accuracy_calibration_skipped`). `headphone_family` is the bounded `HeadphoneFamily` bucket (`airpods_pro`/`airpods_max`/`airpods`/`beats`/`other`/`unknown`/`none`), never the raw device name - the raw name is persisted only with the climb record, never as an analytics parameter |
