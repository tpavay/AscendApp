...
| Fitness | `headphone_family_at_start`, `headphone_output_connected_at_start`, `motion_capable_headphone_connected_at_start`, `headphone_family_at_save`, `headphone_output_connected_at_save`, `motion_capable_headphone_connected_at_save`, `headphone_changed_during_climb`, `headphone_motion_data_flowed`, `app_steps_bucket`, `machine_steps_bucket`, `discrepancy_abs_bucket`, `discrepancy_percent_bucket` | `WorkoutStepAccuracyAnalyticsEvent` (`step_accuracy_telemetry_recorded`, `step_accuracy_calibration_submitted`, `step_accuracy_calibration_skipped`). Captured at both recording-start and save time, since a save-time-only read misattributes a climb where headphones disconnected or switched mid-climb. `headphone_family_at_*` is the bounded `HeadphoneFamily` bucket (`airpods_pro`/`airpods_max`/`airpods`/`beats`/`other`/`unknown`/`none`), never the raw device name - the raw name is persisted only with the climb record, never as an analytics parameter |
| Purchase History | `placement`, `paywall_identifier`, `paywall_name`, `presented_by`, `is_free_trial_available`, `presentation_id`, `presentation_source_type`, `primary_product_id`, `product_id`, `transaction_type`, `restore_type`, `error_type`, `dismiss_reason`, `entitlement_id`, `entitlement_active`, `outcome` | `paywall_*` and `revenuecat_*` events built by `PaywallAnalyticsEvent`, plus `onboarding_paywall_reached` |
...
| Product Interaction | `screen_id`, `step_id`, `from_step`, `flow_id`, `flow_version`, `segment_id`, `step_index`, `step_count`, `resume`, `completion_reason`, `action_id`, `input_type`, `selection_type`, `answer_index`, `has_answer`, `viewed`, `completed`, `question_id`, `provider`, `source`, `entry_point`, `surface`, `share_surface`, `card_type`, `home_state`, `action_state`, `can_start`, `blocked_reason`, `selection_method`, `total_climbs_bucket`, `filter_group`, `filter_type`, `metric`, `time_frame`, `active_filter_count`, `has_active_filters`, `session_id`, `session_type`, `root_route`, `auth_state`, `screen_name`, `screen_class` | Navigation, funnel position, interaction shape, and bounded app-session context across every feature. `screen_name` and `screen_class` ride the one `screen_view` event and come from the bounded `TelemetryScreenName` catalog, so they are route labels rather than anything the climber typed |
...
| Other Diagnostic Data | `holds_sandbox_entitlement`, `storekit_receipt_name` | The StoreKit environment pair `StoreKitEnvironmentDiagnostics` merges onto every typed `PaywallAnalyticsEvent` and onto raw `paywall_error`, `paywall_skipped`, and `paywall_reached` records through `PaywallAnalyticsEvent.diagnosticRecord`. A boolean about the running install and the receipt file name iOS handed it (`sandboxReceipt`, `receipt`, or `none`) - device environment facts that name no product, price, account, or purchase, which is why they are Diagnostic rather than Purchase History |
| Other Diagnostic Data | `gate_attempt_id`, `restore_attempt_id`, `restore_source`, `provider_outcome`, `recovery_path`, `recovery_reason`, `identity_match` | Gate and restore events use bounded app-generated correlation identifiers so one start can join to exactly one terminal without logging a Firebase uid or identity revision. Restore source, recovery path, recovery reason, provider outcome, and identity match are closed enums or booleans. They never include raw provider errors, receipt contents, products, or prices |
| Other Diagnostic Data | `reason` on `paywall_skipped` and `onboarding_auth_failed`, `error_type` on `paywall_error`, `expected_offering_id`, `has_expected_offering`, `missing_product_count` | `paywall_skipped.reason` is a value from the closed `SuperwallSkipReasonCategory` set, never `String(describing:)` output. `paywall_error.error_type` is the fixed `presentation_error` category. The remaining fields describe the bounded RevenueCat catalog audit in `RevenueCatEntitlementService` |
...
- `reason` is **Other Diagnostic Data** on `paywall_skipped`, where it is one of `holdout`, `no_rule_match`, `frequency_capped`, `placement_disabled`, or `unknown`, and on `onboarding_auth_failed`, where it carries an SDK-authored string rather than a user choice.
...
Failed terminals add the bounded `error_type` and deliberately omit `entitlement_active`, because an unavailable answer is not evidence that access is inactive.
