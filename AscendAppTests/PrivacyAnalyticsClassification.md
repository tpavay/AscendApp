...
| Other Diagnostic Data | `holds_sandbox_entitlement`, `storekit_receipt_name` | The StoreKit environment pair `StoreKitEnvironmentDiagnostics` merges onto every event `PaywallAnalyticsEvent` builds - every `revenuecat_purchase_*` and `revenuecat_restore_*` event plus the `paywall_*` events cased in that enum. The raw-`TelemetryRecord` `paywall_error`, `paywall_skipped`, and `paywall_reached` do not carry the pair (#508). A boolean about the running install and the receipt file name iOS handed it (`sandboxReceipt`, `receipt`, or `none`) - device environment facts that name no product, price, account, or purchase, which is why they are Diagnostic rather than Purchase History |
