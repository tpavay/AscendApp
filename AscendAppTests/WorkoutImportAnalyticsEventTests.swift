...
struct WorkoutImportAnalyticsEventTests {
    @Test
    func startedUsesAppleHealthOnlySourceMixForAppleHealthCandidate() {
        let candidate = ImportedWorkoutCandidate.appleHealth(sample: makeAppleHealthSample(id: "ah_123"))
    ...
    @Test
    func finishedUsesPartialSuccessForAppleHealthBatch() {
        let candidates = [
            ImportedWorkoutCandidate.appleHealth(sample: makeAppleHealthSample(id: "ah_1")),
            ImportedWorkoutCandidate.appleHealth(sample: makeAppleHealthSample(id: "ah_2"))
        ]

        let result = ImportBatchResult(
            importedWorkouts: [makeWorkout()],
            updatedWorkouts: [],
            failedCandidateIDs: ["ah_2"]
        )
