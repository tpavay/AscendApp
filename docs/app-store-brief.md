...
- **Apple Health (optional)** - attaches heart-rate and energy samples to a climb performed
  in Ascend, for that climb's own time window. Manual workout logging and Apple Health
  workout import are being removed (#437), so neither is a feature to sell.
