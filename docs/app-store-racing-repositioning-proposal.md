...
Kept: connecting Apple Health so heart-rate and energy samples attach to a climb performed *in Ascend*, live heart rate from a Bluetooth chest strap, Live Climbs, leaderboards, First Ascents, Best Efforts, guided routines, and the share composer.
The legal pages describe the same read set, deliberately stated as heart-rate and energy samples rather than pinned to a HealthKit identifier list, so the disclosure does not drift as #353 trims the read set to exactly what enrichment needs.
