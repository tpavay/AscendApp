...
- [ ] AC-2: Entitlement routing distinguishes `.unknown` from `.inactive`, and `.unknown` resolves to a loading route rather than the paywall. Already satisfied on `develop` by `AppRootRouteResolver` (added in #168); this change keeps it honest with a contract test rather than re-implementing it.
...
| AC-2 | `AppRootRouteResolverTests.resolvesWhileAccessStateIsUnknown` and `MonetizationIdentityTransitionStateTests.routingSeparatesUnknownAccessFromDeniedAccess`. | Fails if unknown is treated as paywall/no-access. |
...
## Reconciliation against `develop`

This work was first written on a branch cut from `main`, which is behind `develop` in monetization.
Re-cut onto `develop`, two things changed:

- The `.unknown` -> `.resolving` routing hunk is **redundant**. `develop` already resolves it that way in `AppRootRouteResolver` (arrived via #168), so only the contract test was kept.
- The Debug/Staging paywall-bypass tripwire drafted for `main` is **obsolete**. #271 replaced the `#if DEBUG || STAGING` default with the `AscendAllowsUnentitledAppAccess` Info.plist key, defaulting to false, and a preflight rejects a Release build that sets it to YES. All main-app configurations now gate on entitlement, which is why the captain could reproduce this on Staging.

The identity race itself was **not** fixed on `develop`: its `EntitlementServicing` still exposed unserialized `identify(userId:)` / `resetIdentity()` with no transition tokens. That, the sign-in affordance, the placeholder rewrite, and the failed-resolution recovery are the substance of this change.
