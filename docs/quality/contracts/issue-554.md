...
- [ ] AC-16: Purchase, restore, paywall, watchdog, fallback, and stale-callback analytics emit one privacy-safe terminal per attempt with build, placement, presentation, provider outcome, identity-generation match, entitlement presence, StoreKit receipt environment, and recovery path, without logging user identifiers, receipt contents, prices, or raw provider errors.
...
| Active billing grace | Treat RevenueCat's active entitlement as access and route to the main app. | StoreKit lifecycle evidence plus entitlement-state test. |
| Expired or revoked | Keep the user gated and offer subscription options and recovery actions. | StoreKit lifecycle evidence plus root-route tests. |
...
| Sign-out, account deletion, or account switch during work | Cancel owned tasks, invalidate the attempt generation, ignore every late callback, and resolve only for the new identity. | Adversarial identity-transition tests. |
...
| AC-5 | `MonetizationIdentityTransitionStateTests`, `AppAccessPaywallCoordinatorTests`, and `AppAccessRestoreServiceTests`. | It proves old purchase, restore, presentation, and entitlement work cannot affect the current account or attempt. |
...
| AC-10 | `MonetizationManagerPaywallTests`, `RevenueCatEntitlementServiceTests`, `StoreKitSubscriptionLifecycleTests`, and `AppReviewSandboxEntitlementTests`. | It proves subscribers enter and non-subscribers remain gated across lifecycle changes and StoreKit environments. |
...
| AC-14 | `AscendSubscriptions.storekit`, `StoreKitSubscriptionLifecycleTests`, `NativeSubscriptionPlanMapperTests`, `PaywallPurchaseAnalyticsContractTests`, and `AppAccessPaywallCoordinatorTests`. | It proves Apple lifecycle states are deterministic at the layers the simulator owns before sandbox testing. |
...
| AC-16 | `PaywallPurchaseAnalyticsContractTests`, `PaywallPurchaseAnalyticsTranscriptTests`, `AppAccessPaywallCoordinatorTests`, and `PrivacyAnalyticsClassification.md`. | It proves incidents can be reconstructed without collecting private purchase or identity data. |
