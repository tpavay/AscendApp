...
| Empty | No achievements: the shelf becomes the existing activation state. | `PublicProfileAchievementRenderingTests.absentAchievementsRenderNoPublicAchievementShell`. |
...
`ProfileAchievementLadder` makes that difference explicit: the record-backed initializer produces `placements`, the banded initializer produces `nil`, and `ProfilePrestigeToken.leaderboardTokens` emits the placement badges only when `placements` exists.
...
- `public-profile-achievements-loaded-champion.png` - another climber's shelf.
