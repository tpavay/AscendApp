...
- **Never hand a comparison surface one climber's ladder.** `viewer.achievements` and `otherUser.achievements` sit on the same `ProfileSnapshot`; passing only the other's is how the section came to hide a decorated climber's own trophies behind a brand-new climber's emptiness (#542). The section hides only when *neither* side holds a badge either side can answer for.
- **Every badge Ascend draws is one entry in `ProfileAchievementCatalogue.all`, and array order is display order everywhere.** Adding a badge is an entry, removing one is deleting it, and scoping one to a screen is its `surfaces` set - which is why First Ascents appear on the own profile and not in the comparison. Never re-introduce a per-badge `if` block or a hand-prepended token.
  Its `count` closure returns `Int?`, and that is the correctness rule rather than tidiness: `nil` means *this ladder cannot speak to this badge*, `0` means zero and we know it. A comparison row is emitted only when both sides return non-nil, so a ghost never claims a zero nobody read.
...
- **An empty side is the normal case, and it reads as a slot, not a failure.** Ghosted art via `ProfilePrestigeBadgeArtwork(isEarned: false)` over a real `0` - never a dash, which on the comparison screen means *we do not know this*, and never consolation copy under a scoreboard. That artwork view is the single home for "not earned yet", so the queued own-profile locked-ladder idea adopts it rather than inventing a second visual language for the same five badges.
...
- **The ladder is First Ascents, CHAMPION, #2, #3, TOP 10, TOP 100, and a badge never renders a count it cannot prove.** The TOP 3 band badge was retired on 2026-08-11 because an exact placement says more than the band containing it.
  ...
  `ProfileAchievementLadder` is where that distinction lives - the record-backed initializer yields `placements`, the banded one yields `nil` - so never reconstruct a placement from a band anywhere else. Rationale: `docs/quality/contracts/podium-placement-badge-ladder.md`.
...
- **Every shelf badge is free-standing cut-out art: transparent, `template-rendering-intent: original`, and 512px on the long edge.** `ProfilePrestigeBadgeView` therefore never clips or strokes the artwork - a tile would slice the cut-out and a stroke would outline transparency - and the token's tint survives only as the glow behind it.
