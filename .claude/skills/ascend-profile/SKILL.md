...
- Both modes render the same achievement ladder through the same shelf, `ProfilePrestigeBadgeShelf`, which takes the achievement records as an optional: a record set - even an empty one - makes each badge open its history sheet, and `nil` renders plain art.
