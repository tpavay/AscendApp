...
- Discovery only reaches the `users/{uid}` subtree. User-keyed data stored outside it needs an explicit step in the sweep, so top-level `notification_devices`, `leaderboard_stats`, `userRateLimits`, `live_replay_leaderboards/{contextKey}/finishers/{uid}`, the `firstAscent*` fields on `live_replay_leaderboards/{contextKey}`, `feedback`, and the uid-keyed `email_jobs` each get one. Add a step whenever a new collection denormalizes a user's display fields, delivery tokens, or email.
