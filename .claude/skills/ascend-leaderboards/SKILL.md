...
- **The derivation owns the five currently OPEN period documents and nothing else.** It may rewrite or delete only `leaderboardDocumentId(uid, timeFrame, currentPeriod(timeFrame, now).key)`. A closed period's row is the historical record `finalizeLeaderboardAchievements` reads at 00:15 UTC to freeze permanent achievements; deleting it during the window between the period closing and the finalizer running destroys a legitimate climber's earned award and shifts every other climber's rank in that period. Never widen the delete set to "any row for this user".
