...
- **A screenshot and a view hierarchy are attached to crashes, not to ordinary errors.** `SentryCrashContextPolicy` declines both for handled non-fatal errors, because producing either renders the live UI synchronously on the main thread and Ascend has 22 `recordError` call sites. A non-fatal issue with no screenshot is working as designed - do not read it as a lost attachment. Both are masked when they do arrive. There is no session replay and there is not meant to be: it was built on this branch and dropped, because on-error mode records the whole session after the first error and buffer mode renders the screen on the main thread once a second in every session (`docs/sentry-setup.md`). Do not go looking for a replay, and do not turn one on to investigate an issue. Everything legible in the two attachments is painted out - open them for layout, navigation state, and which surface failed, never expecting to read a value off one.
