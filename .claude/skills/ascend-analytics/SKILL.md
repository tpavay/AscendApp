...
- **A step reports its view once per *pass*, not once per view instance.** The dedupe set lives on the persisted, account-owned `PassState` in `OnboardingFlowAnalyticsCoordinator`, so back-navigation, re-renders, relaunches **and route changes** all re-emit nothing; `OnboardingScreenViewRecorder` is stateless and just asks `claimScreenView(stepID:)`. It cannot be view state: the paywall is a different root route from onboarding, so walking back from it destroys any `@State` guard - which is exactly the navigation the guard has to survive. Retiring the pass (account change, sign-out of the adopting account, debug replay) is what makes a step reportable again. A claim is only kept for a view that was actually delivered: both `track(_:)` and `track(_:ifIdentifiedAs:)` report whether they delivered - collection being off refuses either, a moved identity refuses the second - and `OnboardingScreenViewRecorder` hands the claim back on a refusal so the step is not deduped against a view nothing ever reported.
...
- **Only `OnboardingFlowAnalyticsCoordinator` owns the lifecycle pair.** It starts at the first onboarding screen the install shows - welcome on a clean run, the resumed step when a reinstall keeps the Keychain session but not `UserDefaults` - persists that start across relaunch, and completes only after active access routes the app to Home. A pass that opens past the first canonical step says `resume=true` on its start, and every launch that picks up a pass already under way emits exactly one `onboarding_flow_resumed` for the screen it comes back to. No screen view carries `resume`: a step reports its view once per pass, so there is no re-emitted view left for the flag to ride on.
...
- **Every RevenueCat purchase and restore emits exactly one terminal, and `completed` means verified.**
  A purchase reports `started` only when a RevenueCat call actually happens, so a missing StoreKit product or an unconfigured purchase emits the failure terminal alone, and that lone terminal carries no `placement` and no `presentation_id` because no presentation ever reached it (`RevenueCatPurchaseAttribution`).
  A restore emits `revenuecat_restore_started` at the user-attempt boundary before the configuration guard, then an unconfigured restore emits exactly one configuration failure terminal for that same attempt.
  ...
  `revenuecat_purchase_completed` requires active `app_access` in RevenueCat state proven for the exact captured identity generation.
  The CustomerInfo returned by the completed purchase is authoritative for that device verdict when it already contains active `app_access`; a second CustomerInfo request is only a bounded propagation path when the purchase response is not active.
  Firebase reconciliation is asynchronous server-projection repair and never delays or downgrades that verified RevenueCat terminal.
  ...
  That budget is **one 10-second total** (`MonetizationVerdictBudget`) spanning identity serialization and the RevenueCat customer-info verdict only, never one per segment; Firebase reconciliation begins after the device verdict and is outside that budget.
...
- **Every event `PaywallAnalyticsEvent` builds also carries the StoreKit environment pair**, `holds_sandbox_entitlement` and `storekit_receipt_name`, merged in one place by `PaywallAnalyticsEvent.record(diagnostics:)` from `StoreKitEnvironmentDiagnostics` - never added per call site, so a new case cannot ship without them.
  ...
  Raw `paywall_error`, `paywall_skipped`, and `paywall_reached` records must be built through `PaywallAnalyticsEvent.diagnosticRecord` so they carry the same StoreKit pair.
  The app-access coordinator also emits one `app_access_gate_attempt_terminal` per hosted attempt with bounded provider outcome, recovery path, identity match, and entitlement-presence fields.
