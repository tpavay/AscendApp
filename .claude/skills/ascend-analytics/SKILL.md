...
- **Every RevenueCat purchase and restore emits exactly one terminal, and `completed` means verified.**
  ...
  A refresh that established nothing is reported as a failure with the matching bounded `error_type` - unconfigured, unresolved identity (including an answer that landed after the identity moved on), the 10-second identity-wait deadline expiring, or RevenueCat unreachable - and the stored entitlement is never read in its place. `MonetizationEntitlementRefresh` is what makes that distinction expressible.
