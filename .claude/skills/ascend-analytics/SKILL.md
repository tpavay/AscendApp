...
- `clearUserId()` returns whether it actually cleared. That return value is the app's only answer to "did somebody sign out", so anything else reporting a sign-out - the `auth:sign_out` breadcrumb included - hangs off it rather than off the listener's `nil`.
