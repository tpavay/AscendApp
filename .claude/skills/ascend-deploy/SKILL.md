...
- `ios-verify` - gated on the iOS source/project paths plus `SharedTestVectors/**`, because the Swift halves of the cross-language parity suites read those vectors directly and would otherwise skip the PRs that break them.
  It runs `scripts/ci/run-ios-test-passes.sh <simulator-udid>` with `-scheme "AscendApp-Staging" -configuration Staging ENABLE_TESTABILITY=YES`, which compiles once with `build-for-testing` and then runs the suite as several `test-without-building` passes, each in a fresh host process.
  The passes are derived from `xcodebuild -enumerate-tests` and balanced by test count in `scripts/ci/split-enumerated-tests.mjs`, never from a checked-in partition - a hand-maintained list goes stale the first time a suite is added, and a suite that silently stops running leaves the check green.
...
`scripts/ci/run-ios-test-passes.sh` owns the measurements; the short version is that the whole suite in one host process peaks at 4,228 MB RSS against a `macos-15` runner holding ~7 GB with ~2.4 GB already wired, so it reaches the tail of the run with 65-83 MB free, ~2.7 GB compressed and ~50,000 swapouts, and then stops completing tests entirely.
The demand is concentrated in one place: `ShareStatClusterPresetEvidenceTests` peaks at 2,008 MB by itself, essentially all of it in `addTimeLayoutScalesLinearlyAsHeaviestClustersPileUp`, which runs 108 full `ImageRenderer` rasterizations (four cluster counts x nine interleaved pairs at 390x845, plus nine at export size 1080x2340) with nothing released between them - 1 test 729 MB, 9 tests 915 MB, 2,341 MB once that test is in.
`autoreleasepool` around every render moves the peak by 0 MB, so do not reach for it; that suite's profile is a standing defect and belongs in its own change.
So the job spreads the suite over several fresh host processes, and the cap is 60 minutes only to afford the extra host launches (~4 minutes each), never as a cure - a killed run reached 1,814 completions in 6m44s against a green run's 1,846 in 8m30s, so more minutes buy a longer crawl.
Read a failure by its signature, not its colour: a `timeout-minutes` kill reports as `cancelled`, an exhausted host reports "Test crashed with signal trap" for every test queued on the hosted-window gate at once, and the crash report says `EXC_BREAKPOINT` with `ktriageinfo` reading "mach_vm_allocate_kernel failed within call to vm_map_enter" - an allocation failure, so the crash lands on a different test each run and names none of them.
The script collects that report and prints the free and compressor counts either side of each pass; both exist because two earlier theories (hosted-test runtime, and build residue held into the first pass) survived only until something measured them.
