...
- The Firestore index gate reads current composite and field-override serving states through the authenticated client in `scripts/ci/wait-for-firestore-indexes.mjs`. Do not use `firebase firestore:operations:list --token` for this gate while CLI 15.22.1 is pinned: that command omits the authentication hook, ignores `--token` on a clean runner, and can spend the whole timeout retrying a poll that never reached Firestore. A timeout must retain the last exact index signatures and states.
