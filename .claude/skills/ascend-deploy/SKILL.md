...
The screen-render evidence suites are the other shape, measured 2026-09-03: each holds 1,000-1,434 MB and keeps it, because a rendered screen is retained for the life of the host, so a balanced pass that holds enough of them dies at its tail - pass 2 completed 928 of 997 tests in four minutes, then went silent for 27 minutes with 13 render suites in flight and 67 MB free. The heaviest share one serial isolated host of their own, because each extra host costs ~2 minutes and one pass per suite would not fit - and the movie suite's host also carries `ShareStatClusterPickerEvidenceTests`, last in source order, because it leaves a SwiftData observer that traps on the next hosted save in the same host (bisected locally 2026-09-03; the trap hangs the host inside the crash handler, which is the wedge signature), a test defect filed rather than fixed; the per-suite peaks are recorded beside the list, and `STATIC_REMAINDER_SUITES` beside it names the half of the remaining render suites that keeps the complement pass light.
