...
- `ios-verify` - gated on the iOS source/project paths plus `SharedTestVectors/**`, because the Swift halves of the cross-language parity suites read those vectors directly and would otherwise skip the PRs that break them.
  ...
  Both iOS jobs run `scripts/ci/ensure-watchos-runtime.sh` before `xcodebuild`, and so does each deploy pipeline's `build-ios` job before its Fastlane archive - the watch app ships inside the same upload, so every job that builds a watch-embedding scheme needs the runtime. That is a hard precondition, not an optimisation: any scheme embedding an Apple Watch app refuses to build at all without the simulator runtime matching the **SDK** - not the deployment target, and whatever the destination. The runner image's runtime set changes without notice and the jobs pin `xcode-version: latest-stable`, so the required version moves whenever the runner's Xcode does; the step provisions rather than assuming, in the same defensive style as `Select simulator`. It matches on the SDK's `major.minor` against the parsed `simctl` runtime list - a differing patch still satisfies the scheme, a differing minor does not, and the plain listing also reports unavailable runtimes as though they were installed. A cold download is several minutes and about 4 GB.
- `ios-verify-release` - the only job that compiles Release and the only place `CODE_SIGNING_ALLOWED=NO` appears, paired with `-scheme "AscendApp" -configuration Release -destination "generic/platform=iOS"`. It exists so Release-only build errors surface on the PR instead of on the production deploy.
  **It passes no `-sdk` flag, and must not regain one.** `-sdk <platform>` overrides `SDKROOT` for *every* target in the build, including the embedded `AscendWatch` app, which is then embedded anyway while the build still reports `** BUILD SUCCEEDED **`. Measured here on Xcode 26.3: with `SUPPORTED_PLATFORMS = "watchos watchsimulator"` on the watch target the override is refused and the embedded binary stays `platform WATCHOS`; drop that setting and the same command produces `platform IOS` inside `Watch/`. So two independent things have to hold, neither self-announcing, which is why `scripts/ci/assert-embedded-watch-platform.sh` asserts the *outcome* on the built bundle rather than either input. A green build that ships an iOS binary inside `Watch/` uploads, and the first symptom is an App Store Connect rejection or a watch app that never launches.
  ...
  Beside the watch-platform assertion it runs `scripts/ci/assert-app-icon-present.mjs` against the built bundle and against `Watch/AscendWatch.app` inside it.
...
Given an IPA the script checks both shipped bundles, the installable app and the watch app nested under `Payload/<app>.app/Watch/`, selecting each `Info.plist` exactly rather than by wildcard for the reason in the Fastlane section.
An IPA carrying **no** watch app passes, with a `::warning::` annotation on the run summary: this guard's job is that the icon Apple rejects is present, not that the watch app ships, and whether the watch app rides along in the 1.0 submission is an open product decision - a hard requirement here would fail the deploy after the archive is already built.
...
- The embedded watch app is a second signed bundle and is wired through the lanes exactly like the widget extension: its identifier reaches `match_options_for`, `AscendWatch` is listed in `update_code_signing_settings`, and the export plist's `provisioningProfiles` map names it - `-exportArchive` refuses an archive it cannot resolve a profile for. `AscendWatch` pins `PROVISIONING_PROFILE_SPECIFIER[sdk=watchos*]` to `match AppStore com.TylerPavay.AscendApp.staging.watch` (Staging) and `match AppStore com.TylerPavay.AscendApp.watch` (Release); both profiles exist in the signing repository.
...
- **Neither lane may identify an artifact by wildcard - not the exported IPA, not the app bundle inside it.**
  ...
  Inside the IPA the same rule applies for the same reason: the watch app carries its own `Info.plist`, so `assert-mixpanel-bundle.mjs` and `verify-ipa-build-number.sh` each select `Payload/<app>.app/Info.plist` exactly and refuse anything but one match, rather than reading the first `Payload/*.app/Info.plist` - that wildcard is what broke the Mixpanel check the day the watch app was embedded.
