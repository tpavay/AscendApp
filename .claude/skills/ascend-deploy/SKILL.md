...
- `ios-verify` - gated on the iOS source/project paths plus `SharedTestVectors/**`, because the Swift halves of the cross-language parity suites read those vectors directly and would otherwise skip the PRs that break them.
  ...
  The app still compiles exactly once per job. See "What threatens `ios-verify`'s 45-minute cap" below for why the suite is split at all.
...
**What threatens `ios-verify`'s 45-minute cap is memory, not minutes, and the cap is the symptom rather than the constraint.**
`scripts/ci/run-ios-test-passes.sh` owns the measurements; the short version is that the test host's memory grows with the tests it has already executed and is never reclaimed - 850 tests peak at 2,020 MB RSS, the full 1,927 at 4,228 MB - against a `macos-15` runner holding ~7 GB with ~2.4 GB already wired.
That is why the job spreads the suite over several host processes instead of one, and why the cap stays at 45 minutes: a killed run reached 1,814 completions in 6m44s where a green run took 8m30s for 1,846, so it is *faster* and then stops dead. More minutes buy a longer crawl on a thrashing machine.
Read the failure by its signature, not its colour: a `timeout-minutes` kill reports as `cancelled`, and the run's own resource snapshot is the evidence - the three killed runs each show 65-83 MB free, ~2.7 GB in the compressor, ~50,000 swapouts, and 12-22 minutes of total output silence with the host still alive and logging.
Compare started against finished counts in the uploaded log too, and note that a `.hostsAWindow` test's reported duration is mostly queue time on the single shared `HostedWindowGate` rather than render cost - ~47 suites serialize on it, so "passed after 282 seconds" cannot be read as 282 seconds of work.
An earlier version of this paragraph blamed hosted-test *runtime* and named PR 549 as the branch that moved it; both were wrong, and the second is measurable: skipping that branch's three suites moved peak RSS from 4,228 MB to 4,343 MB, i.e. nowhere.
