...
- `ios-verify` - gated on the iOS source/project paths plus `SharedTestVectors/**`, because the Swift halves of the cross-language parity suites read those vectors directly and would otherwise skip the PRs that break them.
  ...
  The passes are derived from `xcodebuild -enumerate-tests` and balanced by test count in `scripts/ci/split-enumerated-tests.mjs`, never from a checked-in partition - a hand-maintained list goes stale the first time a suite is added, and a suite that silently stops running leaves the check green. The one exception is `ISOLATED_SUITES`, a named suite too heavy to share a host, which is fail-loud for exactly that reason.
...
The job runs the suite as two balanced `test-without-building` passes off one build, plus one short pass per isolated suite.
