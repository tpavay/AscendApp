...
Both `build-ios` jobs run `scripts/ci/assert-monetization-keys-configured.mjs <Staging|Release>` before the archive: a placeholder RevenueCat or Superwall key builds and uploads cleanly, so the gate has to fire before Fastlane rather than after. `docs/superwall-paywall-setup.md` owns the per-environment key split and the replacement checklist.
