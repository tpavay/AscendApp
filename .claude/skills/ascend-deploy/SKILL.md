...
- `scripts-verify` - installs `scripts/` (`npm --prefix scripts ci`), runs the `scripts/test/*.test.mjs` suite with `node --test`, then audits the `scripts/` lockfile (`--package-lock-only`).
  ...
  The job is gated on changes to `scripts/**` or `SharedTestVectors/**`, plus the iOS paths the monetization build-configuration suite reads directly, `AscendWatch/**` because the watch target-configuration suite reads that bundle's `Info.plist` directly alongside the project file, the Firebase configuration files the structural-validation suite reads directly, and the web, legal, and guidance paths the subscription launch-offer suite reads directly.
...
- `ios-verify` - gated on the iOS source/project paths plus `SharedTestVectors/**`, because the Swift halves of the cross-language parity suites read those vectors directly and would otherwise skip the PRs that break them.
  ...
  Both iOS jobs run `scripts/ci/ensure-watchos-runtime.sh` before `xcodebuild`. That is a hard precondition, not an optimisation: any scheme embedding an Apple Watch app refuses to build at all without the simulator runtime matching the **SDK** - not the deployment target, and whatever the destination. The runner image's runtime set changes without notice and the jobs pin `xcode-version: latest-stable`, so the required version moves whenever the runner's Xcode does; the step provisions rather than assuming, in the same defensive style as `Select simulator`. It matches on the SDK's `major.minor` against the parsed `simctl` runtime list - a differing patch still satisfies the scheme, a differing minor does not, and the plain listing also reports unavailable runtimes as though they were installed. A cold download is several minutes and about 4 GB.
...
- **The embedded watch app is not wired into the lanes yet, and the next real deploy fails at signing until it is.** The `AscendWatch` target pins `PROVISIONING_PROFILE_SPECIFIER[sdk=watchos*]` to `match AppStore com.TylerPavay.AscendApp.staging.watchkitapp` (Staging) and `match AppStore com.TylerPavay.AscendApp.watchkitapp` (Release), and neither profile exists.
  Three things are outstanding, in this order: the captain runs `match` locally for the two watch bundle IDs, because CI is `readonly` and cannot create them; `build_staging` / `build_production` pass the watch bundle ID to `match_options_for` so the lane syncs that profile, and list `AscendWatch` in `update_code_signing_settings` so it gets the same resolved certificate name the app and widget do; and the export plist's `provisioningProfiles` map gains the watch bundle ID, or `-exportArchive` refuses an archive it cannot resolve a profile for.
  ...
  CI is unaffected: `ios-verify-release` builds with `CODE_SIGNING_ALLOWED=NO`.
