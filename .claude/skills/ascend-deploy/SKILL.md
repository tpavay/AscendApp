...
- `functions-verify` - installs `functions/`, then lints, unit-tests, runs the emulator-backed suite, and audits (`npm --prefix functions ci`, `run lint`, `test`, `run test:emulator`, `npx audit-ci@7.1.0 --config functions/audit-ci.json --directory functions`).
...
- `shellcheck-verify` - runs the runner image's own `shellcheck` (no install step) against every tracked `*.sh` file the repository holds, found with `git ls-files -z -- "*.sh"` rather than a fixed directory list, so a script added outside `scripts/` is still covered. It runs at ShellCheck's own default severity (`style`, its most inclusive level) rather than narrowing to `warning` or above, since every tracked script already passes at that level. Gated on the same `scripts` filter as `scripts-verify` below, since every `.sh` file today lives under `scripts/**`; a script added somewhere that filter does not cover would need its own path added there. Deliberately dependency-free like `swiftdata-schema-verify`, for the same reason: a shell-script-only change should never be blocked by an npm advisory.
...
- `ios-verify` - gated on the iOS source/project paths plus `SharedTestVectors/**`, because the Swift halves of the cross-language parity suites read those vectors directly and would otherwise skip the PRs that break them, and plus `scripts/ci/**` and `scripts/lib/**`, because the iOS jobs run those scripts and the modules they import - #570 changed how the suite is split across host processes, was routed as "not iOS", and the one job that could prove it was skipped.
  ...
  The watch target remains a build dependency for 1.2 even though no phone app before 1.2 embeds its product, so a scheme that builds that target **may** still need the runtime - whether it does is unverified.
  The refusal that proved this was a hard precondition (`This scheme builds an embedded Apple Watch app`) keyed off the embed phase, and no release before 1.2 embeds anything, so no evidence in the repository says a dependency-only build still demands an installed runtime - issue #496 records that verification.
...
Every npm project is audit-gated at `--audit-level=low` equivalent severity through a pinned `audit-ci`, not bare `npm audit`, so any newly published advisory fails its verify job unless it is allowlisted with an expiry date in that project's `audit-ci.json`. In the jobs that install and prove code (`functions-verify`, `web-verify`) the audit runs last on purpose, so an advisory cannot hide the lint/test/build results that prove the code itself. Deliberate pins, overrides, and the allowlist mechanism (including its 180-day expiry cap, enforced by `scripts/test/audit-ci-allowlist-contract.test.mjs`) are documented in `docs/dependency-security.md`.
...
The watch app ships in 1.2 and every release before it is submitted without it, a decision owned by `docs/heart-rate-zones-plan.md`, so zero embedded watch bundles is the **normal** case for every build before 1.2 rather than an edge case or an act of leniency - do not tighten this branch into a hard requirement, which would fail the deploy after the archive is already built.
...
- The retained watch target is built and signed as a dependency for 1.2, but no phone app before 1.2 embeds its product.
  Its identifier reaches `match_options_for`, `AscendWatch` is listed in `update_code_signing_settings`, and the export plist's `provisioningProfiles` map keeps the 1.2 signing configuration ready.
...
- **Neither lane may identify an artifact by wildcard - not the exported IPA, not the app bundle inside it.**
  ...
  No IPA before 1.2 nests anything, so the wildcard would read correctly today and go on doing so until the 1.2 build that re-embeds; the exact selection stays.
