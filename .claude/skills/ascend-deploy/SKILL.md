...
The screen-render evidence suites are the other shape, measured 2026-09-03: each holds 896-1,434 MB and keeps it, because a rendered screen is retained for the life of the host, so a balanced pass that holds enough of them dies at its tail - pass 2 completed 928 of 997 tests in four minutes, then went silent for 27 minutes with 13 render suites in flight and 67 MB free. The heaviest share one serial isolated host of their own, because each extra host costs ~2 minutes and one pass per suite would not fit - and the movie suite's host also carries `ShareStatClusterPickerEvidenceTests`, last in source order, because it leaves a SwiftData observer that traps on the next hosted save in the same host (bisected locally 2026-09-03; the trap hangs the host inside the crash handler, which is the wedge signature), a test defect filed rather than fixed; the per-suite peaks are recorded beside the list, and `STATIC_REMAINDER_SUITES` beside it names the half of the remaining render suites that keeps the complement pass light.
...
The job runs the suite as one `test-without-building` pass per isolated group, one for the static half of the remainder, and one for everything not named, all off one build.
...
The helper reads copy presence and absence off the accessibility tree, reads pixels at 1x inside a closure that releases them, keeps OCR only for legibility contracts, and writes the 3x photograph only when `ASCEND_EVIDENCE_DIR` is set - which CI never does.
Measured with the same 10 Hz `ps -o rss` sampler on the same machine, isolated per suite: every OCR suite dropped 25-55% (`EmailPreferencesScreenSnapshotTests` 1,412 -> 831 MB, `AppAccessPaywallPlaceholderSnapshotTests` 1,357 -> 613 MB), the untouched `SentryMaskingEvidenceTests` mask proof stayed at ~1,050 MB, and the whole remainder in one host peaked at 1,485 MB against 2,929 MB before.
Two hazards that run exposed are not memory: a wheel `DatePicker` walked with the accessibility runtime on materialises every drum row (634 -> 2,560 MB on one screen, so the shared walker stops at `UIPickerView`/`UIDatePicker`), and a `ModelContext.save()` right after a `@Query` screen is torn down can wedge the main thread inside SwiftData's SwiftUI observer at 0% CPU with the host alive - the silent-hang signature - which `-test-timeouts-enabled` turns into a named failure instead of a cancelled job.
