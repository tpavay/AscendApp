...
- **`Workout.sourceMetadata` (the `HeadphoneMotionWorkoutMetadata` JSON bag) needs no migration to extend - it's a free-form string - but it is not a free budget.** `firestore.rules` rejects the whole workout write once the serialized string exceeds 4000 characters, with no client-side guard anywhere and no telemetry that would catch it: a long climb (~90 min, a 360-entry split-curve array, several tracking-gap step corrections from a flaky Bluetooth connection) already sits close to or over that limit from the existing fields alone, before adding anything. A new field there is cheap only if it stays small and bounded. `TimeInterval`/`Double` fields are rounded (hundredths of a second for durations, one decimal for the step-accuracy discrepancy percent) rather than encoded at full floating-point precision, and `stepCorrections` is capped to the most recent 20 entries at serialization time - both cheap ways to claw back headroom, both worth reusing before adding a genuinely new unbounded field. The 360-entry `splitSteps` cap itself lives in `LiveReplaySplitSampler` (`maxCheckpoints`), unrelated to this file.
