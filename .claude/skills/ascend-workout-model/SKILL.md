...
- **`Workout.sourceMetadata` (the `HeadphoneMotionWorkoutMetadata` JSON bag) needs no migration to extend - it's a free-form string - but it is not a free budget.** `firestore.rules` rejects the whole workout write once the serialized string exceeds 4000 characters (`WorkoutRemoteSyncLimits.maximumSourceMetadataLength`). The one client-side guard is `HeadphoneMotionWorkoutMetadata.jsonString`, which sheds its least essential detail (the raw headphone names, then the oldest step corrections - the exact order is documented on `jsonString`) until the payload fits, and returns an oversized string only when nothing sheddable is left - so a new field that grows without bound still gets the workout refused. A long climb (~90 min, a 360-entry split-curve array, several tracking-gap step corrections) already sits close to that limit from the core fields alone, so a new field there is cheap only if it stays small and bounded. `TimeInterval`/`Double` fields are rounded (hundredths of a second for durations, one decimal for the step-accuracy discrepancy percent), `stepCorrections` is capped to the most recent 20 at construction, and each raw headphone name (captured at recording start and again at save) to 64 characters - reuse those before adding a genuinely new unbounded field. The 360-entry `splitSteps` cap itself lives in `LiveReplaySplitSampler` (`maxCheckpoints`), unrelated to this file.
...
- Plausibility validation runs at the entry boundary, and the gate is the same for an edit and for a Live Climb completion. Be exact about what it actually checks: `WorkoutPlausibilityPolicy` refuses a non-positive duration and an average cadence above 220 steps/min, and nothing else - it does not bound duration from above. A duration no human could have climbed is therefore caught upstream, where it is produced (`ActiveHeadphoneWorkoutDraftStore.maximumCreditableTrackingGap` clamps the one path that can invent one, a crash-recovered draft synced long after it started), and refused at the server by `isPhysicallyPossibleClimb` in `firestore.rules` - see `ascend-firebase-data`. Do not read this gate as covering more than cadence.
...
- **A stop is re-openable without a release.** Two triggers, each keyed on a persisted marker compared for difference so it fires once per change rather than once per pass, and both gated by `workout_sync_recovery_reopen_enabled`. The recovery basis is `buildIdentity|workout_sync_recovery_epoch`; when it moves, every stopped workout gets exactly one more automatic attempt. `firestore.rules` deploys independently of app releases, so the epoch is the only lever that unsticks a fleet after a rules fix - see `docs/remote-config-kill-switches.md`. The climber signing back in re-opens only the series whose recorded blocker is `authentication`, because an `authentication` series outlives a day before it exhausts and those climbs have scrolled into history by the time the sign-in is repaired. Screen appearances, tab switches and a cold-launch session restore deliberately do not qualify; neither does a repaired sign-in for a refused or malformed series, which is a statement about the bytes rather than the credentials.
