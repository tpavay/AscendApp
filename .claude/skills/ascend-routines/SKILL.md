...
- **Past the point where the intervals stop fitting, the whole routine moves to an overview and the strip becomes a working window onto it** - `RoutineTimelineWindow` decides where that point is, and it decides it from the 44pt tap target rather than from a chosen block count, so 334pt of plot holds seven. Fit to width is not reversed; it applies to the window. The threshold is symmetric by decision - the overview arrives on the eighth interval and leaves again on the seventh - and everything that puts a block on screen goes through `start(bringingIntoView:)`, so an add, a delete and a block dragged past the edge all move the window the same way. The overview is a `RoutineRidgeShape` because a path has no minimum width per interval; the same curve draws the detail screen's hero.
...
- **Every block is one accessibility element with an adjustable action for the level**, plus rotor actions for time, step type, move and delete - dragging bars is an invented interaction, so VoiceOver needs an equivalent for each gesture. The overview is one more element of its own, adjustable to step the window an interval at a time, because dragging it is the only way a finger moves the window. Strings live in `RoutineIntervalVoiceOver`. Motion respects `accessibilityReduceMotion` through `RoutineTimelineMotion`.
