...
- Routine sessions publish to no replay index today, because the server-side indexer accepts only the `live_climb` and `just_climb` tracking modes. Any future routine leaderboard publisher must gate on `target_reached` rather than invent a parallel eligibility rule.
