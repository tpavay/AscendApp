...
class UnifiedImportService {
    ...
    func importWorkout(_ pending: PendingWorkout, skipPRRecalculation: Bool = false) async -> Bool {
        guard let modelContext = modelContext else { return false }

        switch pending {
        case .appleHealth(let hkWorkout):
            return await importAppleHealthWorkout(hkWorkout, modelContext: modelContext, skipPRRecalculation: skipPRRecalculation)

        case .hevy(let exercise, let matchingAHWorkout):
            return await importHevyExercise(exercise, matchingAHWorkout: matchingAHWorkout, modelContext: modelContext, skipPRRecalculation: skipPRRecalculation)
        }
    }

    /// Imports all pending workouts
    func importAllWorkouts() async -> Int {
        ...
        for pending in sortedWorkouts {
            if await importWorkout(pending, skipPRRecalculation: true) {
                successCount += 1
            }
        }
    ...
    private func importAppleHealthWorkout(_ hkWorkout: HKWorkout, modelContext: ModelContext, skipPRRecalculation: Bool) async -> Bool {
        do {
            ...
            // Remove from pending
            pendingWorkouts.removeAll { $0.id == "ah_\(hkWorkout.uuid.uuidString)" }
    ...
    private func importHevyExercise(_ exercise: HevyExerciseHistory, matchingAHWorkout: HKWorkout?, modelContext: ModelContext, skipPRRecalculation: Bool) async -> Bool {
        ...
        if let existingWorkout = findWorkoutByHevyId(exercise.workout_id, modelContext: modelContext) {
            return updateExistingWorkout(existingWorkout, with: exercise, modelContext: modelContext)
        }
        ...
        do {
            ...
            pendingWorkouts.removeAll { $0.id == "hevy_\(exercise.workout_id)" }
