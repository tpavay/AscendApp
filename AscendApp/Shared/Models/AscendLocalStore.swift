...
enum AscendLocalStore {
    ...
    /// The store shape the app writes today. A new `AscendSchemaV5` changes this one line, plus
    /// its stage in `AscendMigrationPlan`.
    static var currentSchema: any VersionedSchema.Type { AscendSchemaV4.self }
