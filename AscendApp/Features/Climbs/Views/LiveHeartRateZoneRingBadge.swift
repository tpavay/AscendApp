...
/// The Just Me tab's heart-rate readout, drawn inside the centered stat grid's heart-rate box
/// (`LiveClimbJustMeView.heartRateCard`): a ring split into the three live effort zones
/// (`HeartRateZone`) with the current BPM centered inside, colored to match. This is the only
/// heart-rate surface Just Me draws - it replaces `LiveHeartRateStatusChip` there entirely
/// rather than sitting alongside it. The Leaderboard tab is unaffected and keeps the chip.
struct LiveHeartRateZoneRingBadge: View {
    let status: LiveHeartRateStatus
    /// The stat grid's box draws this considerably larger than the badge's original top-chrome
    /// size, so its BPM number carries the same visual weight as the 24-26pt values in the
    /// grid's other boxes - stroke width and content size scale with it rather than staying
    /// fixed inside a bigger circle.
    ...
    var body: some View {
        ZStack {
            Circle()
                .stroke(Color.white.opacity(0.16), style: StrokeStyle(lineWidth: lineWidth))
    ...
    private var content: some View {
        ...
        case .connected(let beatsPerMinute, let zone):
            Text(beatsPerMinute.formatted())
                .font(.montserratBold(size: contentFontSize))
                .monospacedDigit()
